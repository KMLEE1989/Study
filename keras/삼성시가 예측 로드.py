...
# print(np.unique(closing_price_samsung))
# print(np.unique(closing_price_kioom))

x1=df1.drop(range(893,1120), axis=0)
#print(x1.info) 
#print(x1.shape) #(893, 17)


x2=df2.drop(range(893,1060), axis=0)
#print(x2.info)
#print(x2.shape) #(893, 17)
...
#print(x1.describe)  일자   시가   고가    저가    종가 전일비  Unnamed: 6   등락률     거래량  금액(백만)   신용비     개인     기관  외인(수량)    외국계   프로그램    외인비
#print(x2.describe)  일자   시가   고가    저가    종가 전일비  Unnamed: 6   등락률     거래량  금액(백만)   신용비     개인     기관  외인(수량)    외국계   프로그램    외인비

x1=x1.drop(columns=['일자', "Unnamed: 6", '전일비' , '등락률','금액(백만)', '신용비' , '개인', '기관','외인(수량)', '외국계' , '프로그램' , '외인비','거래량'], axis=1)
x2=x2.drop(columns=['일자', "Unnamed: 6", '전일비' , '등락률','금액(백만)', '신용비' , '개인', '기관','외인(수량)', '외국계' , '프로그램' , '외인비', '거래량'], axis=1)
# print(x1.info) #시가     고가     저가     종가
# print(x2.info) #시가     고가     저가     종가
...
#print(x1.shape,x2.shape,y1.shape,y2.shape) #(893, 4) (893, 4) (1120,) (1060,)

def split_xy3(dataset, time_steps, y_column):
    x,y=list(), list()
    for i in range(len(dataset)):
        x_end_number=i+time_steps  #5
        y_end_number=x_end_number+y_column-1 #9
        
        if y_end_number>len(dataset):
            break
        tmp_x=dataset[i:x_end_number, 1:] #0 1 2 3 4  1: 
        tmp_y=dataset[x_end_number-1:y_end_number,0] #4:9
        x.append(tmp_x)
        y.append(tmp_y)
    return np.array(x), np.array(y)

xx1,yy1=split_xy3(x1,5,5)
xx2,yy2=split_xy3(x2,5,5)

# print(xx1,"\n",yy1)
# print(xx2,"\n",yy2)


#print(x1.shape, y1.shape) (885, 5, 3) (885, 5)
#print(x2.shape, y2.shape) (885, 5, 3) (885, 5) 

from sklearn.preprocessing import MinMaxScaler, StandardScaler, RobustScaler, MaxAbsScaler

from sklearn.model_selection import train_test_split

x1_train, x1_test, x2_train, x2_test, y1_train, y1_test, y2_train, y2_test = train_test_split(xx1,xx2,yy1,yy2, train_size=0.8, random_state=66)

#print(x1_train.shape) (708, 5, 3)
...
start=time.time()
model.fit([x1_train, x2_train],[y1_train, y2_train], epochs=100, batch_size=16, validation_split=0.2, verbose=1, callbacks=[es, mcp]) 
end=time.time()-start
...
#model.save_weights("../study/_save/samsung stock price 시가 가중치 제출.h5")
'''
model=load_model("../study/_ModelCheckPoint/삼성시가예측가중치.hdf5")

loss=model.evaluate([x1_test, x2_test], [y1_test, y2_test], batch_size=1)

pred1 = np.array(x1[-5:,1:]).reshape(1,5,3)
pred2 = np.array(x2[-5:,1:]).reshape(1,5,3)   #4: 남겨논 칼럼의 수
# print(pred1)
...
print('삼성전자 12/22 시가 : ', result1[-1][-1],'WON')
print('키움증권 12/22 시가 : ', result2[-1][-1],'WON')

# print(y1_pred[-1], y2_pred[-1])

# print('삼성예측종가: ' , y1_pred[-1][-1])
# print('키움예측종가: ' , y2_pred[-1][-1])
