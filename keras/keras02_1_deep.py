...
model.add(Dense(3))  #시퀀셜이기 때문에 인풋을 쓸 필요가 없어
model.add(Dense(4))
model.add(Dense(2))
model.add(Dense(1))  #인공신경망 완성 

#3. 컴파일,훈련
model.compile(loss='mse', optimizer='adam')  #그은선과 실제 데이터의 거리 loss  민스쿼드에라 평균 제곱(하는 이유 양수화 음수가 나올수 없다) 에라 (작으면 좋다)    옵티마이저(엠에스이를 최소화 )

model.fit(x, y, epochs=100, batch_size=1) #핏은 훈련을 시키다  에포 몇번 선을 긋겠다   배치가 작을수록 훈련은 잘돼 (속도차이) (#3이 끝나면 최적의 웨이트를 만들 준비 ) 
