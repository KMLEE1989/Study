...
#print(pad_x.shape) #(1, 4)


word_size = len(token.word_index)
#print("word_size: ", word_size)  #4
...
#                                                   인풋은(13,4)
#                  단어사전의 개수                     단어수,길이
model.add(Embedding(input_dim=1, output_dim=10, input_length=4))  #텐서플로우의 inputdim 여기서는 아웃풋이 아니다 유일하게  13,5,28->28*10
#model.add(Embedding(28,10, input_length=5))
#model.add(Embedding(28,10))
model.add(LSTM(32))  #<- 통상 아웃풋 근데 임베딩에서는 달라
model.add(Dense(1, activation='sigmoid'))
