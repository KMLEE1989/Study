...
Trading_volume_samsung=df1['종가'].unique()

closing_price_kioom=df2['종가'].unique()
opening_price_kioom=df2['종가'].unique()
Trading_volume_kioom=df2['종가'].unique()
...
x1=x1.drop(columns=['일자','종가', "Unnamed: 6", '전일비' , '등락률','금액(백만)', '신용비' , '개인', '기관','외인(수량)', '외국계' , '프로그램' , '외인비'], axis=1)
x2=x2.drop(columns=['일자','종가', "Unnamed: 6", '전일비' , '등락률','금액(백만)', '신용비' , '개인', '기관','외인(수량)', '외국계' , '프로그램' , '외인비'], axis=1)
x1=np.array(x1)
x2=np.array(x2)
...
# print(x1.shape, y1.shape) (889, 5, 3) (889, 1)
# print(x2.shape, y2.shape) (889, 5, 3) (889, 1)
...
dense1=LSTM(8, activation='relu', name='dense1')(input1)
dense2=Dense(5, activation='relu', name='dense2')(dense1)
dense3=Dense(5, activation='relu', name='dense3')(dense2)
output1=Dense(1, activation='relu', name='dense4')(dense3)

input2=Input(shape=(5,3))
dense11=LSTM(7, activation='relu', name='dense11')(input2)
dense12=Dense(5, activation='relu', name='dense12')(dense11)
dense13=Dense(3, activation='relu', name='dense13')(dense12)
dense14=Dense(3, activation='relu', name='dense14')(dense13)
...
start=time.time()
model.fit([x1_train, x2_train],[y1_train, y2_train], epochs=100, batch_size=8, validation_split=0.2, verbose=1, callbacks=[es, mcp]) 
end=time.time()-start

print("걸린시간 :" , round(end, 3), '초')

model.save("../study/_save/samsung stock price.h5")
...
# model.compile(loss='mse', optimizer='adam', metrics=['mse'])
# model.fit([x1_train, x2_train],[y1_train, y2_train], epochs=100, batch_size=8, validation_split=0.2, verbose=1)
